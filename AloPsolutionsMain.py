...
# DeleteTable("COGSTable")
# DeleteTable("ReceivablesAccount")
# DeleteTable("CashAccount")
# DeleteTable("PayablesAccount")
# DeleteTable("ClosingStockBalTable")
# DeleteTable("ExpensesTable")
...
# Edit Data in Other Transaction Table
...
# Edit Update Stocks after selling product
...
# Remove Stock Balances from Transactionsafter selling product
...
# Update Closing Stock in Stocks Table
...
# Add Transaction in Expenses Table
...
# Create ClosingStockBalance Tables
...
# Function Add Balance to Closing Stock Table
...
# Function Adding the Closing Balance
def StoreClosingStock():
    ClosingBalance = Business.Sum_ClosingBal_Val(Business)
    ID = str(datetime.now()) +"_"+ str(ClosingBalance)
...
# Record Cash Receipt in Cash Account
...
# Record Cash Payment in Cash Account
...
def TransactionInputForm():
    ...
    def UpdateStocks_FIFO(Item_name,ItemQ):
        # 1. Ids
        ...
        def COGSUpdate():
            # COGS Inputs.....
            ID = str(datetime.now()) +"_" + Values[1]
            ItemName = Values[1]
    ...
    with st.form("TransactionInput",clear_on_submit=True):
        ...
        CustomerSupplier = fil6.text_input("Customer/Supplier Name")
                        ...
                        if "Sale" in ActionType[0]:
                            ...
                            # Check if the balance of product units do not exceed the available balance
                                ...
                                if float(ItemQ) <= TotalBal:
                                    ...
                                    AddTransaction(a=id,b=Item_name,c= -abs(ItemQ),d=UnitPrice,e=ActionType[0],
                                                f=TransDateNotime,g=ItemMeasure[0],h= -abs(TotalValue),i=description,j=TrnsDate,k=CustomerSupplier)
                                    ...
                                    AddTransactionOtherTable(tablename="SalesTable",a=id,b=Item_name,c=ItemQ,d=UnitPrice,e=ActionType[0],
                                                            f=TransDateNotime,g=ItemMeasure[0],h=TotalValue,i=description,j=TrnsDate)
                                    UpdateStocks_FIFO(Item_name,ItemQ)
                                    StoreClosingStock()
                            ...
                            def PurchaseUpdates():
                                AddTransaction(a=id,b=Item_name,c= ItemQ,d=UnitPrice,e=ActionType[0],
                                            f=TransDateNotime,g=ItemMeasure[0],h= TotalValue,i=description,j=TrnsDate,k=CustomerSupplier)
                                st.success(f" The {ActionType[0]} for {Item_name} has been recorded successful!")
                                st.snow()
...
def TransactionEditForm():
                    
    TransactionID = Business.SelectionFromTable("Business","TransactTable","ID")
    IDs_in_SalesTable = Business.SelectionFromTable("Business","SalesTable","ID")

    ProductPickList = Business.SelectionFromTable(self=Business,tablename="ItemTable",colName="ItemName")
    ProductMeasure = Business.SelectionFromTable(self=Business,tablename="ItemTable",colName="ItemMeasure")
    #TransTypeList = Business.SelectionFromTable(self=Business,tablename="TransactionTypeTable",colName="ActionName")
    TransTypeList = ["Cash Purchase","Cash Sale","Credit Purchase","Credit Sale"]

    Transaction = st.multiselect("Pick Transaction to Edit",TransactionID)
    if len (Transaction) == 1:
        ...
        sel_transdate = data[0][5]
        sel_itemmeasure = data[0][6]
        sel_descrip = data[0][8]
        
        if sel_itemname in ProductPickList:
       # A form to edit Transactions details
            with st.form("TransactionEdit",clear_on_submit=False):
                fil1,fil2,fil3 = st.columns(3)
                fil4,fil5,fil6 = st.columns(3)
                
                new_itemname = fil1.multiselect(label="Item Name",default=sel_itemname,options=ProductPickList)
                new_itemq= fil2.text_input(label="Product Quantity",value=sel_itemq)
                new_unitprice = fil3.text_input(label="Product Unit Price",value=sel_unitprice)
                new_actiontype = fil4.multiselect(label="Transaction Type",default=sel_actiontype,options=TransTypeList)
                new_itemmeasure = fil5.multiselect(label="Product Unit Measure",default=sel_itemmeasure,options=ProductMeasure)
                new_descrip = st.text_area(label="Transaction Description",value=sel_descrip)
                sub_btn = st.form_submit_button("Click to Update Transaction")

                if sub_btn:
                   
                    if len(new_itemname) == 1 and len(new_itemq) > 0 and len(new_unitprice) > 0 and len(new_actiontype) == 1 and len(new_itemmeasure)== 1 and len(new_descrip) > 1:
                        
                        if isnumber(num=new_itemq) and isnumber(num=new_unitprice):
                            sel_list = [sel_itemname,sel_itemq,sel_unitprice,sel_actiontype,sel_itemmeasure,sel_descrip]
                            new_list = [new_itemname[0],float(new_itemq),float(new_unitprice),new_actiontype[0],new_itemmeasure[0],new_descrip]
                        
                            if collections.Counter(sel_list) != collections.Counter(new_list):
                                Itemname = new_itemname[0]
                                TotalValue = float(new_unitprice) * float(new_itemq)
                                
                                new_itemq = float(new_itemq)
                                TotalValue = float(TotalValue)

                                if "Sale" in new_actiontype[0]:

                                    AvailProductsStocks = Business.SelectionFromTable(self=Business,tablename="StocksTable",colName="ItemName")
                                    if Itemname in AvailProductsStocks:
                                                
                                    # Check if the balance of product units do not exceed the available balance
                                        TotalBal = Business.Sum_Values(self=Business,colVal="ItemQuantity",tablename="TransactTable",refcol="ItemName",refval=Itemname)[0][0]
                                                
                                        if float(new_itemq) <= TotalBal:
                                    
                                            # Updates in all Transactions Table
                                            EditTransactionFunc(new_itemname[0],-abs(new_itemq),new_unitprice,new_actiontype[0],new_itemmeasure[0],new_descrip,-abs(TotalValue),fil_ID)
                                            st.success(f"Congratulations! You have successful updated transactions for: {Itemname}")

                                            if fil_ID in IDs_in_SalesTable:
                                                EditOtherTransactionFunc(tablename="SalesTable",new_itemname=new_itemname[0],new_itemq=new_itemq,
                                                                        new_unitprice=new_unitprice,new_actiontype=new_actiontype[0],new_itemmeasure=new_itemmeasure[0],
                                                                        new_descrip=new_descrip,TotalValue=TotalValue,fil_ID=fil_ID)
                                            else:
                                                AddTransactionOtherTable(tablename="SalesTable",a=fil_ID,b=new_itemname[0],c=new_itemq,d=new_unitprice,e=new_actiontype[0],
                                                                        f=sel_transdate,g=new_itemmeasure[0],h=TotalValue,i=new_descrip)
                                            # Delete Transaction in StocksTable after transfering it to Sales Table
                                            Business.DeleteItem(self=Business,tablename="StocksTable",refcol="ID",refval=fil_ID)
                                        
                                        elif float(new_itemq) > TotalBal and TotalBal > 0:
                                             st.warning(f"You can not sell {int(new_itemq)} units for the Product ({Itemname}). The maximum possible selling amount as per available balance is  {int(TotalBal)} units only")
                                        elif float(new_itemq) > TotalBal and TotalBal <= 0:
                                            st.warning(f"Sorry! You can not sell {int(new_itemq)} units of {Itemname} for now as the product has no balance!")
                                        else:
                                            st.warning(f"The Product ({Itemname}) you are trying to sell has a balance of only {TotalBal}. Please increase the stock or sell a different product")
                                    else:
                                        st.warning(f"Sorry! There is no stock for the Product ({Itemname}) you are trying to sell.")

                                elif "Purchase" in new_actiontype[0]:
                                    ...
                                    if fil_ID in IDs_in_SalesTable:
                                        EditOtherTransactionFunc(tablename="StocksTable",new_itemname=new_itemname[0],new_itemq=new_itemq,
                                                                new_unitprice=new_unitprice,new_actiontype=new_actiontype[0],new_itemmeasure=new_itemmeasure[0],
                                                                new_descrip=new_descrip,TotalValue=TotalValue,fil_ID=fil_ID)
                                    else:
                                        AddTransactionOtherTable(tablename="StocksTable",a=fil_ID,b=new_itemname[0],c=new_itemq,d=new_unitprice,e=new_actiontype[0],
                                                                f=sel_transdate,g=new_itemmeasure[0],h=TotalValue,i=new_descrip)

                                    # Delete Transaction in SalesTabe after transfering it to Stocks Table
                                    Business.DeleteItem(self=Business,tablename="SalesTable",refcol="ID",refval=fil_ID)
                            else:
                                st.error(f"No changes commited on Transaction for:: {sel_itemname}")
                        else:
                            st.warning("Product Quantity and Product Unit Price should be numbers. Please correct & repost!")       
                    else:
                        st.warning("⚠️To proceed make sure that all the fields in the form above are completed with one one value!")   
        else:
            st.warning(f"Transaction you are trying to update contain a product ({sel_itemname}) which is not in our database! Add the product before you proceed")
    ...
    if len (Transaction) == 1:
        ...
        # A form to edit Transactions details
            with st.form("TransactionDelete",clear_on_submit=False):
                ...
                fil1.multiselect(label="Item Name",default=sel_itemname,options=ProductPickList,disabled=True)
                fil2.text_input(label="Product Quantity",value=sel_itemq,disabled=True)
                fil3.text_input(label="Product Unit Price",value=sel_unitprice,disabled=True)
                fil4.multiselect(label="Transaction Type",default=sel_actiontype,options=TransTypeList,disabled=True)
                fil5.multiselect(label="Product Unit Measure",default=sel_itemmeasure,options=ProductMeasure,disabled=True)
                st.text_area(label="Transaction Description",value=sel_descrip,disabled=True)
                sub_btn = st.form_submit_button("Click to DELETE the Transaction Above")
                ...
                if sub_btn:
                    ...
                    if "Sale" in sel_actiontype:
                        Business.DeleteItem(self=Business,tablename="SalesTable",refcol="ID",refval=fil_ID)
                    elif "Purchase" in sel_actiontype:
                        Business.DeleteItem(self=Business,tablename="StocksTable",refcol="ID",refval=fil_ID)
